...
| 特性                 | 5.0  | 5.1  | 5.2  | 5.3  | 5.4  |
| ------------------ | ---- | ---- | ---- | ---- | ---- |
| **发布年份**           | 2016 | 2019 | 2020 | 2021 | 2023 |
| **2 M PHY**        | ✅    | ✅    | ✅    | ✅    | ✅    |
| **长距（LE Coded）**   | ✅    | ✅    | ✅    | ✅    | ✅    |
| **测向定位 (AoA/AoD)** | ❌    | ✅    | ✅    | ✅    | ✅    |
| **LE Audio / LC3** | ❌    | ❌    | ✅    | ✅    | ✅    |
...
- 5.4 带来“无连接双向广播（PAwR）+ 加密广告”，更适合大规模传感器/ESL场景

###### ==Mesh需要关心的版本==

**蓝牙 5.0 与 4.x 在“能否跑 Mesh”这件事上逻辑完全一致**：

1. 蓝牙 Mesh 并不是某一代蓝牙核心规范（4.0、5.0、5.2…）的一部分，而是**2017 年 SIG 另行发布的应用层规范**。  
2. **只要芯片硬件满足 BLE 4.0 及以上**，并有足够的 Flash/RAM 来跑 2017 版 Mesh 协议栈，就能组网。  
3. 因此，**BLE 5.0 芯片并不天然比 BLE 4.2 芯片“更能”Mesh**；它只是多了 LE 2M PHY、LE Coded PHY（长距离）等新物理层特性，**Mesh 仍靠软件协议栈实现**。

一句话：  **“BLE 5.0 芯片能否 Mesh”取决于厂商是否移植了 2017 Mesh 协议栈，而不是蓝牙核心版本本身。**

###### ==BLE（低功耗蓝牙）和低功耗芯片的关系==

BLE指的是低功耗蓝牙技术，协议本身是为了低功耗蓝牙通信所设计的。

但是**支持BLE技术的芯片不一定是低功耗的芯片**。因为芯片SoC的设计目标可能是支持多种功能，而不仅仅是低功耗。他们可能只在蓝牙的时候是低功耗，而在其他模式下功耗较高。整体上并没有针对低功耗做对应的设计。

低功耗芯片指的是专门针对低功耗应用设计的芯片。通常会提供多种低功耗模式和优化技术。
